...
WiFiServer server(80);  //wifi server creates object of server on port 80
...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED) {  //its waiting for connecting wifi, when connected it breaks
    delay(500);
    Serial.print(".");
  }
  Serial.println("\nWiFi connected!");
...
void loop() {
  WiFiClient client = server.available();  //client wo jo ip address paste krain gay
